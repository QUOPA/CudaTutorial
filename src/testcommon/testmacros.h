...
#define REGISTERTESTSUBCASE_CUDA_KERNEL(LEFT,RIGHT, ...)\
do { std::cout << std::endl << "---- TEST SUBCASE : " << #LEFT << "," << #RIGHT << std::endl; \
auto _tickbegin = std::chrono::steady_clock::now();\
try { LEFT,RIGHT(##__VA_ARGS__);  } \
...
#define REGISTERTESTCASE_CUDA_KERNEL(LEFT,RIGHT, ...)\
do { std::cout << std::endl << "-- TEST CASE : " << #LEFT << "," << #RIGHT << std::endl; \
auto _tickbegin = std::chrono::steady_clock::now();\
try { LEFT,RIGHT(##__VA_ARGS__);  } \
catch (std::exception & e) { std::cout << "Exception : " << e.what() <<std::endl; } \
catch (...) { std::cout << "Exception : Unexpected" << std::endl; }\
...
do { std::cout << std::endl << "-- TEST CASE : " << #FN << std::endl; \
auto _tickbegin = std::chrono::steady_clock::now();\
try { FN(##__VA_ARGS__);  } \
catch (std::exception & e) { std::cout << "Exception : " << e.what() <<std::endl; } \
catch (...) { std::cout << "Exception : Unexpected" << std::endl; }\
